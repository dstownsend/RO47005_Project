...
def run_albert(n_steps_max=10000, render=False, goal=True, obstacles=True,input_pose = None, final_pose=None):
    ...
    waypoints = global_planner(input_pose=input_pose,final_pose=final_pose,n_waypoints=10)
    ...
    while run and n_it < n_steps_max:
        ...
        # something wrong with theta possibly, check out the global and local orientations
        x,y,theta = pos[0],pos[1],orn[2]
        
        current_state = np.array([x,y,theta])

        u,w = local_planner(waypoints,current_state)

        action[0],action[1] = u,w
