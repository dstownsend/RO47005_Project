...
MID_WALL_HEIGHT = "high" # low, middle, high
...
def main():
    ...
    for height in range(3):
        ...
        if height == 0:
            ...
            MID_WALL_HEIGHT = "middle"
        else:
            MID_WALL_HEIGHT = "high"

        utc_ms = int(time.time() * 1000)
        result = f"{utc_ms},{MID_WALL_HEIGHT}\n"
        ...
        for iteration in range(10):
            ...
            utc_ms = int(time.time() * 1000)
            result = f"{utc_ms}, SEQUENCE START {iteration}\n"
            with open("ArmResults.txt", "a", encoding="utf-8") as file:
                file.write(result)
            ...
            for step in range(N_STEPS):
                if phase == "move_base":
                    ...
                    if not arm_controller.goal_reached:
                        ...
                        utc_ms = int(time.time() * 1000)
                        result = f"{utc_ms},SEQUENCE COMPLETE\n"
                        with open("ArmResults.txt", "a", encoding="utf-8") as file:
                            file.write(result)
                ...
                if terminated:
                    ...
                    utc_ms = int(time.time() * 1000)
                    result = f"{utc_ms},Arm Cubic, {info}\n"
                    with open("ArmResults.txt", "a", encoding="utf-8") as file:
                        file.write(result)
